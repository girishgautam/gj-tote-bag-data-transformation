...
resource "aws_lambda_layer_version" "transformation_utils_layer" {
  layer_name = "utils"
  compatible_runtimes = ["python3.12"]
  # s3_bucket = aws_s3_bucket.code_bucket.bucket
  # s3_key = aws_s3_object.transformation_utils.key
  filename = "${path.module}/../packages/transformation_utils/utils.zip"
  source_code_hash = filebase64sha256("${path.module}/../packages/transformation_utils/utils.zip")
}
